...
# RTX 5060 호환성(CUDA error) 임시 해결을 위한 CPU 강제 할당
...
class Config:
    ...
    FORCED_STEER_PHASE1_MIN_DUR = 0.3  # 1단계: 진입 최소 유지 시간 (노이즈 방지)
...
def get_steering_angle(camera, img, model, config, lane_state="RIGHT"):
    ...
    if right_x_base is None:
        if len(search_half) > 0 and np.max(search_half) > 0:
            ...
            # 아예 안 보이면 이전 값 유지, 없으면 기본값
            right_x_base = config.PREV_RIGHT_X if config.PREV_RIGHT_X is not None else (
                limit_max - config.OFFSET_FROM_LINE if lane_state == "RIGHT" else limit_min + config.OFFSET_FROM_LINE
            )
...
def main():
    ...
    print("🚀 CPU 추론 엔진 초기화 중... (수 초가 소요될 수 있습니다)")
    # 빈 이미지를 생성하여 첫 번째 추론을 미리 수행합니다. (CPU 메모리 할당)
    dummy_img = np.zeros((480, 640, 3), dtype=np.uint8)
    model_right.predict(source=dummy_img, imgsz=432, verbose=False)
    # CPU 모드에서는 JIT 컴파일이 없으므로 나머지 모델들은 생략하여 로딩 속도를 높입니다.
    print("✅ CPU 추론 엔진 초기화 완료!")
        ...
        if mission_model is not None:
            ...
            if run_mission_model:
                if not is_waiting_light:
                    # [최강 최적화] 코너링 탈선 방지 로직!
                    # 1. 코너를 돌 때(핸들이 좌우로 12도 이상 꺾였을 때)는 어차피 정지선이 없으므로 정지선 모델을 꺼버리고 조향(30 FPS)에 100% 집중합니다.
                    # 2. 직선 구간이더라도 프레임을 절반(짝수 프레임)으로 나눠서 검사해 CPU 과부하를 막습니다.
                    if abs(getattr(Config, 'PREV_STEER', 90) - 90) < 12 and frame_count % 2 == 0:
                    ...
                    last_results_f0_boxes = [] # 바닥 카메라는 안 봄
