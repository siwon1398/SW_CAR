...
void loop() {
  ...
  // --- 💡 [개선] 오차 기반 동적 속도 계산 (스톨 방지 로직 적용) ---
  int angle_error = abs(target_angle - current_angle);
  int dynamic_steer_speed;

  if (target_angle <= 70 || target_angle >= 110) {
    // 🚨 강제 회피(Phase 1, 2): 목표가 물리적 한계치 바깥일 때
    
    // 1. 목표치에 도달할 때까지는 감속 없이 강한 펀치력(150)으로 순식간에 회전
    dynamic_steer_speed = 150; 
    
    // 2. 바퀴가 물리적 한계점(70, 110)에 도달하는 순간 모터 파워 0으로 차단 (스톨 방지)
    if ( (target_angle <= 70 && current_angle <= 70 + error_margin) || 
         (target_angle >= 110 && current_angle >= 110 - error_margin) ) {
      dynamic_steer_speed = 0; 
    }
  } 
  else {
    // 🛣️ 일반 주행(Phase 0): 기존 로직과 동일하게 부드러운 주행 유지
    dynamic_steer_speed = map(angle_error, 0, 30, 60, 130);
