...
def main():
    ...
    cv2.namedWindow("ROI_Tuning", cv2.WINDOW_NORMAL)
    cv2.resizeWindow("ROI_Tuning", 400, 200)

    initial_top_width = int(Config.SRC_POINTS[1][0] - Config.SRC_POINTS[0][0])
    initial_bot_width = int(Config.SRC_POINTS[2][0] - Config.SRC_POINTS[3][0])

    cv2.createTrackbar("TOP_WIDTH", "ROI_Tuning", initial_top_width, 640, on_trackbar)
    cv2.createTrackbar("BOT_WIDTH", "ROI_Tuning", initial_bot_width, 640, on_trackbar)
    cv2.createTrackbar("TOP_Y", "ROI_Tuning", int(Config.SRC_POINTS[0][1]), 480, on_trackbar)
    cv2.createTrackbar("BOT_Y", "ROI_Tuning", int(Config.SRC_POINTS[2][1]), 480, on_trackbar)
    ...
    while True:
        ...
        # 트랙바에서 ROI 값 읽어와서 Config 실시간 업데이트
        try:
            top_width = cv2.getTrackbarPos("TOP_WIDTH", "ROI_Tuning")
            bot_width = cv2.getTrackbarPos("BOT_WIDTH", "ROI_Tuning")
            top_y = cv2.getTrackbarPos("TOP_Y", "ROI_Tuning")
            bot_y = cv2.getTrackbarPos("BOT_Y", "ROI_Tuning")

            center_x = 320
            tl_x = center_x - (top_width // 2)
            tr_x = center_x + (top_width // 2)
            bl_x = center_x - (bot_width // 2)
            br_x = center_x + (bot_width // 2)
